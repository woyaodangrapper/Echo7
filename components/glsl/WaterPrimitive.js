...
uniform sampler2D reflectionTexture; // 反射贴图
...
vec3 sunDirection = vec3( 0.0 );//normalize( lightDirection );
...
czm_material czm_getMaterial(czm_materialInput materialInput) {
    ...
    // vec3 reflectionSample = vec3( TEXTURE2D( reflectionTexture, (v_uv.xy / v_uv.w) * 0.5 + 0.5 + distortion ) );
    vec3 reflectionSample = vec3(TEXTURE2D(reflectionTexture, (v_uv.xy / v_uv.w) * 0.5 + 0.5 + distortion));
...
export default class WaterPrimitive {
  ...
  set height(value) {
    ...
    const rwpCa = Cesium.Cartographic.fromCartesian(
  ...
  preRender(scene) {
    ...
    texture.type = "sampler2D";
    // 设置 uniform
    this._material.uniforms.reflectionTexture = texture;
